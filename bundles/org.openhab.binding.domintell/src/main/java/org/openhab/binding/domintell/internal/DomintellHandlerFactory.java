...
public class DomintellHandlerFactory extends BaseThingHandlerFactory {
    ...
    @Activate
    public DomintellHandlerFactory(final @Reference DomintellDiscoveryService discoveryService,
            final @Reference UnitProvider unitProvider) {
        this.discoveryService = discoveryService;
        Unit<Temperature> unit = unitProvider.getUnit(Temperature.class);
        if (unit != null && !SIUnits.CELSIUS.equals(unit)) {
            useCelsius = false;
        }
    }
            ...
            if (SUPPORTED_MODULE_THING_TYPES_UIDS.contains(thingTypeUID)) {
                ...
                switch (thingTypeUID.getId()) {
                    ...
                    case MODULE_PBX:
                        ...
                        break;
                    case MODULE_TEX:
                        handler = new DomintellThermostatModuleHandler(thing, registry, useCelsius);
                        break;
                    case MODULE_D10:
                    case MODULE_DIM:
                    case MODULE_DAL:
